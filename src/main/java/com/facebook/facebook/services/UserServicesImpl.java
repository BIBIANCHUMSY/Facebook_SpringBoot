...
public class UserServicesImpl implements IUserServices {
    ...
    @Override
    public FacebookUser registerUser(FacebookUserRequestDTO newUser) {
        if (userRepository.existsByEmail(newUser.getEmail())) {
            throw new IllegalArgumentException("Email already exists");
        }
        String genderValue = newUser.getGender();

        // Set the converted gender value
        newUser.setGender(genderValue);

        FacebookUser user = mapper.map(newUser, FacebookUser.class);
    ...
    @Override
    public FacebookUser updateUser(FacebookUserRequestDTO oldUser) {

        FacebookUser user = mapper.map(oldUser, FacebookUser.class);
